...
namespace DataManagement.DAO
{
    internal class MsSqlCreation : ICreatable
    {
        ...
        public string CreateSelectStoredProcedure<T>(bool doAlter) where T : Cope<T>, IManageable, new()
        {
            ...
            if (Cope<T>.ModelComposition.FilteredProperties.Count == 0) return string.Empty;

            if (doAlter)
            {
                queryBuilder.AppendFormat("ALTER PROCEDURE {0}.{1}{2}{3}\n", Cope<T>.ModelComposition.Schema, Manager.StoredProcedurePrefix, Cope<T>.ModelComposition.TableName, Manager.SelectSuffix);
            }
            else
            {
                queryBuilder.AppendFormat("CREATE PROCEDURE {0}.{1}{2}{3}\n", Cope<T>.ModelComposition.Schema, Manager.StoredProcedurePrefix, Cope<T>.ModelComposition.TableName, Manager.SelectSuffix);
        ...
        public string GetCreateForeignKeysQuery<T>(Dictionary<string, KeyDefinition> keyDetails = null) where T : Cope<T>, IManageable, new()
        {
            ...
            foreach (KeyValuePair<string, PropertyInfo> property in properties)
            {
                ...
                queryBuilder.AppendFormat("ADD CONSTRAINT FK_{0}_{1}\n", Cope<T>.ModelComposition.TableName, foreignKey.Composition.TableName);
                queryBuilder.AppendFormat("FOREIGN KEY({0}) REFERENCES {1}.{2}{3}(Id) ON DELETE {4} ON UPDATE NO ACTION;\n", property.Value.Name, Cope<T>.ModelComposition.Schema, Manager.TablePrefix, foreignKey.Composition.TableName, foreignAttribute.Action.ToString().Replace("_", " "));
            }
