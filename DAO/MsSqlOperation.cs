        ...
        public Result<T> ExecuteProcedure<T>(T obj, string connectionToUse, TransactionTypes transactionType, bool logTransaction = true) where T : Cope<T>, IManageable, new()
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = Connection.OpenMsSqlConnection(connectionToUse))
                {
                    ...
                    Command = connection.CreateCommand();
                    Command.CommandType = CommandType.StoredProcedure;
                    Command.CommandText = string.Format("{0}.{1}{2}{3}", Manager<T>.ModelComposition.DataTableAttribute.Schema, Manager.StoredProcedurePrefix, Manager<T>.ModelComposition.DataTableAttribute.TableName, GetFriendlyTransactionSuffix(transactionType));
                    ...
                    else
                    {
                        ...
                        dataTable.TableName = Manager<T>.ModelComposition.DataTableAttribute.TableName;
                    }
                }
            ...
            catch (SqlException sqlException) when (sqlException.Number == ERR_OBJECT_NOT_FOUND)
            {
                ...
                {
                    Logger.Warn(string.Format("Table {0} not found. Creating...", Manager<T>.ModelComposition.DataTableAttribute.TableName));
